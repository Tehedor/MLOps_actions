...
check-setup:
	...
	@MLOPS_USE_VENV=$(USE_VENV) $(PYTHON) setup/check_env.py
	@echo "==> Verificando configuración del proyecto"
	@MLOPS_USE_VENV=$(USE_VENV) $(PYTHON) setup/check_setup.py
...
publish-generic: check-variant-format
	...
	@for ext in $(PUBLISH_EXTS); do \
	  find $(VARIANTS_DIR)/$(VARIANT) -type f -name "*.$$ext" -exec $(DVC) add {} + 2>/dev/null || true; \
	done
...
# 	@$(DVC) push -r storage || (echo "[ERROR] dvc push failed"; exit 1)
	@$(DVC) push  || (echo "[ERROR] dvc push failed"; exit 1)
...
variant_dir = Path(meta_path).parent
models_dir = variant_dir / "models"

# Le decimos a MLflow qué experimento usar
mlflow.set_experiment(exp_name)
exp = mlflow.get_experiment_by_name(exp_name)

# Iniciamos el run y registramos todo
...
print(f"[OK] Run MLflow creado con éxito: {run_id}")
endef
...
if not isinstance(data.get("mlflow_registration"), dict):
	data["mlflow_registration"] = {
		...
		"artifacts": [
			f"executions/05_modeling/{variant}/models",
			f"executions/05_modeling/{variant}/05_modeling_metadata.json",
		],
	}
