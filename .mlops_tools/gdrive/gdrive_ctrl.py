...
class GDriveController:
    def __init__(self):
        ...
        if not all([self.client_id, self.client_secret, self.base_folder_id]):
            raise ValueError("Error: Faltan credenciales de GDrive o el ID de la carpeta de la fase en .env")
    ...
    def _authenticate(self):
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                # Construimos la configuración del cliente en memoria para no requerir un credentials.json extra
                client_config = {
                    "installed": {
